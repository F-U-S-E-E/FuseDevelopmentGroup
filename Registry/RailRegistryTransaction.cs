...
namespace RAIL.Registry
{
    ...
    /// </summary>
    public sealed class RailRegistryTransaction : IDisposable
    {
        private readonly string _packageId;
        private readonly KeyValuePair<RailClaimKind, string>[] _snapshot;
        private bool _committed;
        private bool _finished;

        internal RailRegistryTransaction(string packageId)
        {
            _packageId = packageId ?? string.Empty;
            _snapshot = RailRegistry.GetClaimsForPackage(_packageId).ToArray();
            RailRegistry.ReleaseAllForPackage(_packageId);
        }
        ...
        public void Rollback()
        {
            ...
            {
                RailRegistry.TryClaim(pair.Key, pair.Value, _packageId);
            }
        }
