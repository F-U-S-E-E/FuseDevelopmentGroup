#!/usr/bin/env python3
"""
convert_fuse_audio.py - convert loose horn/whistle/bell packs into FUSE audio packages.

This is a clean-room importer: it reads legacy JSON documents as input data only
and writes FUSE-native package files. It does not import legacy code.
"""
...
def package_info(source: Path):
    for name in ("Definition.json", "definition.json", "Info.json", "info.json"):
        ...
        try:
            ...
            continue
        mod_id = data.get("id") or data.get("Id") or slug(source.name, "fuse-audio")
    ...
    return slug(source.name, "fuse-audio"), source.name, "1.0.0", "", {}, None
...
def fuse_skeleton(mod_id: str, mod_name: str, version: str, author: str):
    return {
        "$schema": ".\\schemas\\fuse-mod.schema.json",
        "schemaVersion": FUSE_SCHEMA_VERSION,
        "id": f"{mod_id}.FUSE.Audio",
        "name": f"{mod_name} (FUSE Audio)",
        "author": author,
...
def copy_asset_pack_wrapper(source: Path, mods_out: Path, clean: bool):
    ...
    output = mods_out / f"{source.name}.FUSE"
...
def convert_loose_package(source: Path, mods_out: Path, clean: bool):
    ...
    report = []
    rail = fuse_skeleton(mod_id, name, version, author)
    ...
    info = {
        "$schema": ".\\schemas\\umm-info.schema.json",
        "Id": f"{mod_id}.FUSE",
        "DisplayName": f"{name} (FUSE Audio)",
        "Author": author,
        "Version": str(version),
        "ManagerVersion": "0.27.10",
        "Requirements": ["FUSE"],
        "LoadAfter": ["FUSE"],
        "FuseDataFiles": ["audio.fuse.json"],
    }
    write_json(output / "Info.json", info)
...
def main():
    parser = argparse.ArgumentParser(description="Convert horn/whistle/bell packs to FUSE.")
