...
{
    public sealed class RailApplyReport
    {
        private const int MaxLoggedItemsPerBucket = 60;

        public RailApplyReport(string definitionId, string reason, bool isReapply)
        {
        ...
        {
            RailLog.Info(
                $"RAIL apply report package='{DefinitionId}' operation='apply' reason='{Reason}' reapply={IsReapply} " +
            ...
            LogBucket("warning", Warnings, RailLog.Warning);
            LogBucket("error", Errors, RailLog.Error);
            LogBucket("created", CreatedObjects, RailLog.Info);
            LogBucket("updated", UpdatedObjects, RailLog.Info);
            LogBucket("removed", RemovedObjects, RailLog.Info);
            LogBucket("skipped", SkippedObjects, RailLog.Info);
            LogBucket("post-bind", PostBindValidationResults, RailLog.Info);
        }
        ...
        private void LogBucket(string label, IList<string> items, Action<string> logger)
        {
            ...
            {
                logger($"RAIL apply report {label}: package='{DefinitionId}' operation='apply' {items[index]}");
            }

            if (items.Count > count)
            {
                logger($"RAIL apply report {label}: package='{DefinitionId}' operation='apply' ... {items.Count - count} more item(s) omitted from log.");
            }
        }
    ...
    public sealed class RailApplyTransaction
    {
        public RailApplyTransaction(string definitionId, string reason, bool isReapply)
        {
            Report = new RailApplyReport(definitionId, reason, isReapply);
        }

        public RailApplyReport Report { get; }
        ...
        public void RunPhase(string phase, Action action)
        {
            ...
            RailLog.Info($"RAIL apply phase package='{Report.DefinitionId}' operation='{CurrentPhase}' started.");
            try
            {
                action?.Invoke();
                stopwatch.Stop();
                RailLog.Info($"RAIL apply phase package='{Report.DefinitionId}' operation='{CurrentPhase}' completed in {stopwatch.ElapsedMilliseconds} ms.");
            }
            catch (Exception ex)
            {
                stopwatch.Stop();
                Fatal("phase", CurrentPhase, ex.Message);
                RailLog.Exception($"RAIL apply phase package='{Report.DefinitionId}' operation='{CurrentPhase}' failed after {stopwatch.ElapsedMilliseconds} ms", ex);
            }
            finally
            {
        ...
        public bool TryApply(string kind, string id, bool existsBeforeApply, Action action)
        {
            ...
            catch (Exception ex)
            {
                ...
                RailLog.Exception($"RAIL apply failed package='{Report.DefinitionId}' operation='{CurrentPhase}' kind='{kind}' id='{id}'", ex);
                return false;
            }
        }
        ...
        public bool TryRemove(string kind, string id, Action action)
        {
            ...
            {
                Error(kind, id, ex.Message);
                RailLog.Exception($"RAIL remove failed package='{Report.DefinitionId}' operation='{CurrentPhase}' kind='{kind}' id='{id}'", ex);
                return false;
            }
        }
