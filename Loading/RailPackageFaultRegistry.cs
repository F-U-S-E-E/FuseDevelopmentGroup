...
{
    internal sealed class RailPackageFault
    {
        public RailPackageFault(string packageId, string stage, string message, string details)
        {
    ...
    internal static class RailPackageFaultRegistry
    {
        private static readonly Dictionary<string, List<RailPackageFault>> Faults =
            new Dictionary<string, List<RailPackageFault>>(StringComparer.OrdinalIgnoreCase);
        ...
        public static void RecordFault(string packageId, string stage, string message, Exception exception = null)
        {
            ...
            var fault = new RailPackageFault(packageId, stage, message, details);

            if (!Faults.TryGetValue(packageId, out var packageFaults))
            {
                packageFaults = new List<RailPackageFault>();
            ...
            RailLog.Error($"RAIL package fault recorded: {fault}");
        }
        ...
        public static RailPackageFault[] GetFaults()
        {
        ...
        public static void LogFinalReport(string reason, int residentDefinitionCount)
        {
            ...
            RailLog.Info(
                $"RAIL final package report reason='{reason ?? "unspecified"}' " +
            ...
            foreach (var packageId in faultedPackageIds)
            {
                ...
                {
                    RailLog.Error($"RAIL final package report fault: {fault}");
                }
            }
        }
        ...
        private static void LogSet(string label, IEnumerable<string> values)
        {
            ...
            {
                RailLog.Info($"RAIL final package report {label}: package='{value}'.");
            }
        }
        ...
        private static void LogMap(string label, IDictionary<string, string> values)
        {
            ...
            {
                RailLog.Warning($"RAIL final package report {label}: package='{entry.Key}' reason='{entry.Value}'.");
            }
        }
