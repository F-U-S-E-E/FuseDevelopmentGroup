...
{
    public static class RailWorldSuppressor
    {
        public const string SyntheticAreaFeaturePrefix = "rail.suppression.area.";
        ...
        /// rail.suppression.area.&lt;id&gt;. This keeps the runtime shape close to
        ...
        /// </summary>
        public static void ApplyDefinition(RailModDefinition definition, RailApplyTransaction transaction = null)
        {
            ...
            ClaimSuppressions(definition.Id, groups, RailClaimKind.SuppressedTrackGroup, "track group", transaction);
            ClaimSuppressions(definition.Id, areas, RailClaimKind.SuppressedArea, "area", transaction);
        ...
        {
            if (!RailSettings.EnableExperimentalEarlyScenePathSuppression)
            {
                return;
            }

            foreach (var loaded in RailModLoader.GetLoadedModsInOrder())
            {
        ...
        public static void ApplyActiveScenePathSuppressions(string reason, RailApplyTransaction transaction = null)
        {
            if (!RailSettings.EnableExperimentalEarlyScenePathSuppression)
            {
                return;
            ...
            foreach (var path in GetClaimedIds(RailClaimKind.SuppressedScenePath).ToArray())
            {
        ...
        public static bool HasActiveScenePathSuppressions
        {
            get
            {
                return RailSettings.EnableExperimentalEarlyScenePathSuppression &&
                       GetClaimedIds(RailClaimKind.SuppressedScenePath).Any();
            }
        }
        ...
        public static IReadOnlyCollection<string> GetActiveScenePathSuppressions()
        {
            ...
            return GetClaimedIds(RailClaimKind.SuppressedScenePath)
        ...
        {
            return GetClaimedIds(RailClaimKind.SuppressedTrackGroup)
                .OrderBy(id => id, StringComparer.OrdinalIgnoreCase)
                .ToArray();
        }
        ...
        {
            return GetClaimedIds(RailClaimKind.SuppressedArea)
                .OrderBy(id => id, StringComparer.OrdinalIgnoreCase)
                .ToArray();
        }
        ...
        public static void ApplyActiveAreaSuppressions(string reason, RailApplyTransaction transaction = null)
        {
            foreach (var areaId in GetClaimedIds(RailClaimKind.SuppressedArea).ToArray())
            {
        ...
        public static void ReleasePackage(string packageId)
        {
            ...
            var claims = RailRegistry.GetClaimsForPackage(packageId)
                .Where(pair => pair.Key == RailClaimKind.SuppressedScenePath || pair.Key == RailClaimKind.SuppressedTrackGroup || pair.Key == RailClaimKind.SuppressedArea)
                .ToArray();
            ...
            foreach (var claim in claims)
            {
                try
                {
                    RailRegistry.Release(claim.Key, claim.Value, packageId);
                }
                catch (Exception ex)
                {
                    RailLog.Warning($"RAIL failed to release suppression claim kind='{claim.Key}' id='{claim.Value}' package='{packageId}': {ex.Message}");
                }
            }

            foreach (var path in claims.Where(claim => claim.Key == RailClaimKind.SuppressedScenePath).Select(claim => claim.Value).Distinct(StringComparer.OrdinalIgnoreCase))
            {
                if (RailRegistry.GetSharedOwners(RailClaimKind.SuppressedScenePath, path).Count == 0)
                {
                    RailLog.Info($"RAIL released final scene-path suppression claim for '{path}' during package unload; target remains suppressed until map unload.");
                }
            }

            foreach (var groupId in claims.Where(claim => claim.Key == RailClaimKind.SuppressedTrackGroup).Select(claim => claim.Value).Distinct(StringComparer.OrdinalIgnoreCase))
            {
                if (RailRegistry.GetSharedOwners(RailClaimKind.SuppressedTrackGroup, groupId).Count == 0)
                {
                    RestoreTrackGroup(groupId, "package unload");
                }
            }

            foreach (var areaId in claims.Where(claim => claim.Key == RailClaimKind.SuppressedArea).Select(claim => claim.Value).Distinct(StringComparer.OrdinalIgnoreCase))
            {
                if (RailRegistry.GetSharedOwners(RailClaimKind.SuppressedArea, areaId).Count == 0)
                {
                    RestoreArea(areaId, "package unload");
                }
            }

            RailLog.Info($"RAIL released {claims.Length} world suppression claim(s) for package '{packageId}'.");
        }
        ...
        public static void RestoreAll(string reason)
        {
            ...
            Packages.Clear();
            RailLog.Info($"RAIL restored all world suppressions for '{reason ?? "unspecified"}'.");
        }

        private static void RegisterScenePathSuppressions(string packageId, string[] scenePaths, RailApplyTransaction transaction, string reason)
        {
            ...
            ClaimSuppressions(packageId, scenePaths, RailClaimKind.SuppressedScenePath, "scene path", transaction);
            RailLog.Info($"RAIL registered {scenePaths.Length} experimental scene-path suppression request(s) for package '{packageId}' during '{reason ?? "unspecified"}'.");
        }
        ...
        private static void ApplyActiveTrackGroupSuppressions(string reason, bool rebuildIfChanged)
        {
            ...
            if (graph == null)
            {
                if (GetClaimedIds(RailClaimKind.SuppressedTrackGroup).Any())
                {
                    RailLog.Warning($"RAIL cannot apply track-group suppressions for '{reason ?? "unspecified"}' because Graph.Shared is not available.");
                }
            ...
            var changed = false;
            foreach (var groupId in GetClaimedIds(RailClaimKind.SuppressedTrackGroup).ToArray())
            {
        ...
        private static bool TrySuppressTrackGroup(Graph graph, string groupId, string reason)
        {
            ...
            try
            {
                ...
                if (!known)
                {
                    RailLog.Warning($"RAIL suppressing unknown track group '{groupId}' for '{reason}'. It may be created by another package later.");
                }
                ...
                if (changed)
                {
                    RailLog.Info($"RAIL suppressed base track group '{groupId}' for '{reason}' owners={RailRegistry.GetSharedOwners(RailClaimKind.SuppressedTrackGroup, groupId).Count}.");
                }
            ...
            {
                RailLog.Warning($"RAIL failed to suppress track group '{groupId}' for '{reason}': {ex.Message}");
                return false;
            }
        }

        private static void TrySuppressScenePath(string path, string reason, RailApplyTransaction transaction)
        {
            ...
            try
            {
                var target = RailPrefabResolver.ResolveScenePath(path) ?? GameObject.Find(path);
                if (target == null)
                {
                ...
                if (target.activeSelf)
                {
                    ...
                    RailLog.Info($"RAIL suppressed base scene path '{path}' for '{reason}' owners={RailRegistry.GetSharedOwners(RailClaimKind.SuppressedScenePath, path).Count}.");
        ...
        private static void RebuildAfterTrackGroupSuppression(Graph graph, string reason)
        {
            ...
            try
            {
                ...
                RailLog.Info($"RAIL rebuilt track graph after group suppression for '{reason ?? "unspecified"}'.");
            }
            catch (Exception ex)
            {
                RailLog.Warning($"RAIL failed to rebuild graph after track-group suppression for '{reason ?? "unspecified"}': {ex.Message}");
            }
            finally
            {
        ...
        private static void TrySuppressArea(string areaId, string reason, RailApplyTransaction transaction)
        {
            ...
            try
            {
                ...
                RailLog.Info($"RAIL suppressed base area '{areaId}' for '{reason}' owners={RailRegistry.GetSharedOwners(RailClaimKind.SuppressedArea, areaId).Count} syntheticFeature='{state.FeatureId}'.");
            }
            catch (Exception ex)
            {
        ...
        private static void RestoreTrackGroup(string groupId, string reason)
        {
            ...
            if (graph == null)
            {
                ...
                RailLog.Warning($"RAIL dropped track-group suppression state for '{groupId}' during '{reason}' because Graph.Shared is not available.");
                return;
            }
            ...
            try
            {
                ...
                RailLog.Info($"RAIL restored base track group '{groupId}' for '{reason}' enabled={state.WasEnabled} available={state.WasAvailable}.");
            }
            catch (Exception ex)
            {
                RailLog.Warning($"RAIL failed to restore track group '{groupId}' for '{reason}': {ex.Message}");
            }
        }
        ...
        private static void RestoreScenePath(string path, string reason)
        {
            ...
            try
            {
                var target = state.Target ?? RailPrefabResolver.ResolveScenePath(path) ?? GameObject.Find(path);
                if (target != null)
                {
                    target.SetActive(state.WasActive);
                    RailLog.Info($"RAIL restored base scene path '{path}' for '{reason}' active={state.WasActive}.");
                }
                else
                {
                    RailLog.Warning($"RAIL dropped scene-path suppression state for '{path}' during '{reason}' because the target no longer exists.");
                }
            }
            catch (Exception ex)
            {
                RailLog.Warning($"RAIL failed to restore scene path '{path}' for '{reason}': {ex.Message}");
            }
            finally
            {
        ...
        private static void RestoreArea(string areaId, string reason)
        {
            ...
            foreach (var kvp in state.OriginalProgressionDisabled.ToArray())
            {
                ...
                {
                    RailLog.Warning($"RAIL failed to restore progression-disabled state for area '{areaId}' item='{DescribeDisablable(kvp.Key)}': {ex.Message}");
                }
            }
            ...
            RailLog.Info($"RAIL restored base area '{areaId}' for '{reason}' itemCount={restored} syntheticFeature='{state.FeatureId}'.");
        }
        ...
        private static void RestoreUnusedScenePaths(IEnumerable<string> paths)
        {
            foreach (var path in paths ?? Enumerable.Empty<string>())
            {
                if (RailRegistry.GetSharedOwners(RailClaimKind.SuppressedScenePath, path).Count == 0)
                {
                    RailLog.Info($"RAIL scene-path suppression '{path}' no longer has owners after definition update; target remains suppressed until map unload.");
                }
            }
        }
        ...
        private static void ClaimSuppressions(string packageId, IEnumerable<string> ids, RailClaimKind kind, string label, RailApplyTransaction transaction)
        {
            foreach (var id in ids ?? Enumerable.Empty<string>())
            {
                try
                {
                    ...
                    {
                        transaction?.PostBind("suppression claim", id, $"kind={kind} owners={RailRegistry.GetSharedOwners(kind, id).Count}");
                    }
                    else
                    {
        ...
        private static IReadOnlyCollection<string> GetClaimedIds(RailClaimKind kind)
        {
            return RailRegistry.GetClaimedIds(kind)
                .Where(id => RailRegistry.GetSharedOwners(kind, id).Count > 0)
                .ToArray();
        }
        ...
        private static IEnumerable<IProgressionDisablable> EnumerateAreaDisablables(Area area)
        {
            ...
            {
                RailLog.Warning($"RAIL could not enumerate industries for suppressed area '{area.identifier}': {ex.Message}");
            ...
            {
                RailLog.Warning($"RAIL could not enumerate passenger stops for suppressed area '{area.identifier}': {ex.Message}");
        ...
        private static void EnsureSyntheticAreaFeature(AreaSuppressionState state, Area area)
        {
            ...
            if (manager == null)
            {
                RailLog.Warning($"RAIL could not create synthetic suppression MapFeature '{state.FeatureId}' because MapFeatureManager.Shared is not available.");
                return;
            }
            ...
            try
            {
                ...
                feature.displayName = "RAIL Suppression: " + area.identifier;
                feature.description = "RAIL synthetic area suppression feature. Other mods can filter ids with prefix '" + SyntheticAreaFeaturePrefix + "'.";
            ...
            {
                RailLog.Warning($"RAIL failed to create/register synthetic area MapFeature '{state.FeatureId}': {ex.Message}");
            }
        }
        ...
        private static void RegisterSyntheticFeature(MapFeatureManager manager, MapFeature feature)
        {
            ...
            {
                RailLog.Warning($"RAIL cannot register synthetic MapFeature '{feature.identifier}' because MapFeatureManager._features was not found.");
                return;
            }
        ...
        private static void RemoveSyntheticAreaFeature(AreaSuppressionState state)
        {
            ...
            {
                RailLog.Warning($"RAIL failed to unregister synthetic MapFeature '{state.FeatureId}': {ex.Message}");
            }
            ...
            {
                RailLog.Warning($"RAIL failed to destroy synthetic MapFeature object '{state.FeatureId}': {ex.Message}");
            }
        }
        ...
        private static void NotifyIndustriesChanged(string reason)
        {
            ...
            {
                RailLog.Warning($"RAIL failed to send IndustriesDidChange after {reason}: {ex.Message}");
            }
        }
