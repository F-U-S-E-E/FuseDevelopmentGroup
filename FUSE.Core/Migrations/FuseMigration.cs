...
namespace Fuse.Core.Migrations
{
    /// <summary>
    /// Unity-free mirror of the in-game <c>FUSE.Authoring.Migrations.FuseMigration</c>.
    /// Same schema-version handling, normalization, and compatibility migrations;
    /// the only difference is diagnostics route through an injected
    /// <see cref="IFuseLogger"/> instead of the game-coupled static FuseLog.
    /// <see cref="Normalize"/> never emits warnings (matches the shipping code,
    /// which passes logWarnings:false), so it takes no logger.
    /// </summary>
    public static class FuseMigration
    {
        ...
        public static FuseModDefinition Migrate(FuseModDefinition definition, IFuseLogger logger = null)
        {
            if (definition == null)
            {
            ...
            var log = logger ?? NullFuseLogger.Instance;
            var packageId = GetPackageId(definition);
            var sourceVersion = NormalizeSchemaVersion(definition, packageId, true, log);
            ...
            {
                RunVersionByVersionMigrations(definition, sourceVersion, packageId, log);
            }

            Normalize(definition);
            ApplyCompatibilityMigrations(definition, packageId, log);
            Normalize(definition);
            return definition;
        }
        ...
        public static void Normalize(FuseModDefinition definition)
        {
            ...
            NormalizeSchemaVersion(definition, GetPackageId(definition), false, NullFuseLogger.Instance);
            ...
            foreach (var scenery in definition.World.Scenery.Values)
            {
                ...
                scenery.Scale = scenery.Scale == default ? FuseVector3.one : scenery.Scale;
            }
        ...
        private static void NormalizeProgression(FuseModDefinition definition)
        {
            ...
            foreach (var feature in root.MapFeatures.Values)
            {
                ...
                // Intentionally do NOT coerce null patches to "empty set" here.
                // Under the FuseStringPatch contract, null means "field omitted
                // → keep the live runtime value," while an empty set ([]) means
                // "explicit replace with empty." Downstream consumers are
                // already null-safe.
            }
        }
        ...
        private static void RunVersionByVersionMigrations(FuseModDefinition definition, Version sourceVersion, string packageId, IFuseLogger log)
        {
        ...
        private static void ApplyCompatibilityMigrations(FuseModDefinition definition, string packageId, IFuseLogger log)
        {
        ...
        private static Version NormalizeSchemaVersion(FuseModDefinition definition, string packageId, bool logWarnings, IFuseLogger log)
        {
            Version version;
            string normalized;
        ...
        private static void WarnOnce(IFuseLogger log, string packageId, string code, string message)
        {
            ...
            (log ?? NullFuseLogger.Instance).Warning(message);
        }
        ...
        /// <summary>
        /// Patch-aware alias merge: when exactly one of two aliased patch fields
        /// is populated, surface that one. If both are populated, the explicit
        /// preferred wins (no structural merge across two patches).
        /// </summary>
