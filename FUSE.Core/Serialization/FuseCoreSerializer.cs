...
namespace Fuse.Core.Serialization
{
    /// <summary>
    /// Unity-free mirror of the in-game <c>FUSE.Authoring.Serialization.FuseSerializer</c>.
    /// Same settings, converter stack, migration pipeline (Migrate on read,
    /// Normalize on write) and duplicate-key guard, so <c>*.fuse.json</c> written
    /// here is read identically by the shipping serializer. Verified by the
    /// cross-serializer round-trip contract test in FUSE.Tests.
    /// </summary>
    public static class FuseCoreSerializer
    {
