...
{
    /// <summary>
    /// Registers RAIL's console commands against Railroader's UI.Console.ConsoleCommandHandler.
    ...
    /// we log once and skip — RAIL keeps loading.
    /// </summary>
    internal static class RailConsoleRegistrar
    {
        ...
        public static void TryRegisterAll()
        {
            ...
            try
            {
                ...
                var commands = RailConsoleCommands.CreateAll();
                var registered = 0;
                foreach (var command in commands)
                {
                    ...
                    {
                        RailLog.Warning(
                            $"RAIL console command '{command.GetType().Name}' failed to register: {ex.Message}");
                    }
                }

                _registered = true;
                RailLog.Info($"RAIL registered {registered} console command(s).");
            }
            catch (Exception ex)
            {
                RailLog.Exception("RAIL console registration failed.", ex);
            }
        }
        ...
        private static void LogSurfaceUnavailable(string detail)
        {
            ...
            RailLog.Warning(
                $"RAIL console commands not registered: {detail} " +
                "RAIL will continue to load without console commands.");
        }
