...
Scope: FUSE runtime API, package schema, loader lifecycle, converter tooling, and known legacy mod conversion needs for the Railroader Unity Mod Manager ecosystem.

Status legend: DONE, PARTIAL, IN PROGRESS, BROKEN, MISSING, NEEDS TESTING, NEEDS DOCUMENTATION, UNKNOWN.

## Executive Summary

FUSE is no longer just a schema experiment. The current codebase has a working Unity Mod Manager plugin, separated package discovery/load/apply stages, dependency ordering, runtime apply transactions, package fault reporting, map-load user reports, route/world conversion tooling, map tiles, asset-pack mounting, operations support, progression extensions, world suppression, and loose horn/whistle/bell audio support.
...
| --- | --- | --- | --- | --- |
| Core FUSE API architecture | PARTIAL | High | `API/*.cs`, `Data/FuseModDefinition.cs`, `Serialization/FuseSerializer.cs`, `Migrations/FuseMigration.cs` | Freeze public naming, document supported APIs, add example packages. |
| Mod loading and initialization | DONE | High | `FusePlugin.cs`, `Lifecycle/FuseLifecycle.cs`, `Loading/FuseDataPackageDiscovery.cs` | Keep testing duplicate loads, unloads, and map reloads. |
| Unity Mod Manager integration | DONE | High | `FusePlugin.cs`, `FUSE.csproj`, `Info.json` | Document build and deploy setup for contributors. |
| Route mod loading | PARTIAL | Critical | `tools/fuse_convert.py`, `Loading/FuseModLoader.cs`, converted Appalachian packages | Build a route regression checklist using Appalachian, Oconaluftee, and East Whittier. |
| FUSE asset loading | PARTIAL | High | `Loading/FuseAssetPackRegistry.cs`, `API/FusePrefabResolver.cs`, `tools/convert_fuse_audio.py` | Document asset-pack layouts and unknown asset diagnostics. |
| Track / FUSE systems | PARTIAL | Critical | `API/TrackAPI.cs`, `Loading/FuseTrackRemovalSnapshotStore.cs`, `Validation/FuseDefinitionValidator.cs` | Add span and removal test cases for exact base-game graph IDs. |
| Rolling stock support | PARTIAL | High | `Patches/FuseAudioPatches.cs`, `API/FuseAudioAPI.cs`; no car definition API found | Define whether FUSE will support cars/locos directly or only audio/customization hooks for alpha. |
...
| Map / terrain support | PARTIAL | High | `API/MapAPI.cs`, `Loading/FuseMapTileRegistry.cs`, `Patches/MapStorePatches.cs` | Document map tiles, map labels, speed signs, station icons, and map masks. |
| Save/load behavior | PARTIAL | Critical | `Patches/StateManagerPatches.cs`, `Patches/TrainControllerPatches.cs`, `FuseApiPersistence.cs` | Run save/load/reload tests for route packages and runtime edits. |
| Harmony patching | DONE | High | `Patches/FusePatchResilience.cs`, patch classes | Add a patch report section to troubleshooting docs. |
| Event hooks / lifecycle hooks | PARTIAL | Medium | `Events/FuseEvents.cs`, `Lifecycle/FuseLifecycle.cs` | Document which events are stable and when they fire. |
...
| Compatibility with existing mods | PARTIAL | Critical | `tools/fuse_convert.py`, audio converter, legacy alias normalization | Maintain a conversion test matrix and package-by-package acceptance notes. |
| Error handling and logging | PARTIAL | High | `Infrastructure/FuseLog.cs`, `FusePackageFaultRegistry.cs`, `FuseLoadReport.cs`, `FuseApplyTransaction.cs` | Standardize remaining warnings with package, operation, and object id. |
| Config/settings support | PARTIAL | Medium | `FuseSettings.cs` | Add documented settings model before exposing more feature flags. |
| Dependency handling between mods | DONE | High | `FuseDataPackageDiscovery.cs` supports `FuseLoadPriority`, `FuseLoadAfter`, `FuseLoadBefore` | Document dependency cycles and missing dependency behavior. |
| Documentation quality | PARTIAL | High | `README.md`, `schemas/FUSE_JSON_SCHEMA.md`, generated docs in this audit | Add task-based guides and troubleshooting pages. |
| Examples/sample mods | PARTIAL | Medium | `schemas/fuse-mod.example.json` | Add runnable sample packages under an examples folder. |
| Migration path from Legacy mods to FUSE | PARTIAL | Critical | `tools/fuse_convert.py`, `tools/convert_fuse_audio.py`, converted packages in Mods | Document conversion workflow and known manual-fix items. |

## Detailed Findings

### Core FUSE API Architecture

Current status: PARTIAL

Evidence:
- `Data/FuseModDefinition.cs` defines the top-level package model: tracks, operations, world, audio, progression, editor, and extensions.
- `Serialization/FuseSerializer.cs` supports JSON and BSON with migration on load.
- `Migrations/FuseMigration.cs` normalizes null collections, aliases, component types, progression sections, and schema version.
...
Suggested next action: Publish a "FUSE API alpha contract" doc that names supported public APIs, unstable APIs, and expected call timing.
...
Evidence:
- `FusePlugin.cs` initializes settings, asset packs, Harmony patches, lifecycle, and console commands.
- `Lifecycle/FuseLifecycle.cs` handles map load, graph rebuild, and map unload.
- `Loading/FuseDataPackageDiscovery.cs` separates discovery, disk load, and runtime apply.
...
- Map-load report is surfaced through `FuseLoadReport`.
...
Evidence:
- `FusePlugin.Load` is the Unity Mod Manager entry point.
- `FUSE.csproj` references Unity/Railroader/UMM assemblies and has deploy support with `EnableModDeploy=true`.
...
Evidence:
- `tools/fuse_convert.py` converts legacy JSON files one file per source concern.
- `FuseModLoader` applies tracks, areas, operations, world objects, progression, audio, and suppressions in phased transactions.
- Converted Appalachian route packages are present in the live Mods folder.

Already working:
- Multiple `.fuse.json` fragments per package are supported through `FuseDataFiles`.
...
Evidence:
- `Loading/FuseAssetPackRegistry.cs` mounts asset packs.
- `API/FusePrefabResolver.cs` resolves `vanilla://`, `path://scene`, `scenery://`, and empty prefab references.
- `tools/convert_fuse_audio.py` wraps audio asset-pack-only packages.

Already working:
- Asset-pack folders can be exposed through `FuseAssetPacks`.
- Unknown scenery assets are recorded in `FuseLoadReport`.
...
- Rolling stock asset packs are not yet a first-class FUSE schema target.
...
Suggested next action: Write a guide that maps common legacy asset layouts to FUSE `Info.json`, `FuseAssetPacks`, and `assetIdentifier` usage.
...
- `Validation/FuseDefinitionValidator.cs` validates spans, speed limits, area order, and removal conflicts.
- `Loading/FuseTrackRemovalSnapshotStore.cs` supports reversible track removals.
...
Evidence:
- `API/FuseAudioAPI.cs` registers whistles, horns, and bells.
- `Patches/FuseAudioPatches.cs` patches sound customization and component builders.
- No FUSE data model/API for full car or locomotive definitions was found.

Already working:
- Loose horn, whistle, and bell packs can be converted to FUSE audio packages.
- Custom sound choices can be exposed in the UI through FUSE patches.
- Audio files are copied into FUSE packages by `tools/convert_fuse_audio.py`.
...
Needs to be added:
- Decision: FUSE alpha supports audio customization only, or starts full rolling stock schema.
...
- `Authoring/FuseSceneryEntity.cs` and `FuseConfigurableStructureEntity` route runtime apply through authoring entities.
- `API/FusePrefabSanitizer.cs` sanitizes loaders, stations, turntables, and scene clones.
...
Evidence:
- `Loading/FuseMapTileRegistry.cs` and `Patches/MapStorePatches.cs` mount map tile overlays.
...
- `Loading/FuseWorldSuppressor.cs` handles group, area, and experimental scene-path suppression.
...
- `API/FuseApiPersistence.cs` supports manual and autosave recording scopes.
- `Authoring/FuseAuthoringPersistenceService.cs` can save/capture/apply authoring entities.
...
Evidence:
- `Patches/FusePatchResilience.cs` applies patch classes individually and records failures.
- Patch classes cover map store, state manager, train controller, locations panel, audio, and experimental early loader.

Already working:
- One failed patch should not prevent FUSE from loading.
...
Evidence:
- `Events/FuseEvents.cs` exposes FUSE load/unload, graph rebuild, validation, mod loaded/unloaded, and object mutation events.
- `Lifecycle/FuseLifecycle.cs` wires game lifecycle to FUSE package operations.
...
Evidence:
- `tools/fuse_convert.py` converts AMM/Strange Customs style route JSON.
- `tools/convert_fuse_audio.py` converts loose horn/whistle/bell packs.
- `Data/Operations/FuseIndustryComponentTypes.cs` normalizes legacy component aliases.

Already working:
- Appalachian route data has been converted and iteratively debugged.
- Map tile packages load.
- Several horn/whistle packages convert to FUSE audio.
- Legacy `AlinasMapMod.PaxStationComponent` maps to FUSE `passengerStop`.

Incomplete or risky:
- Legacy route mods can contain object families FUSE only partially supports.
...
Evidence:
- `Infrastructure/FuseLog.cs`, `Loading/FusePackageFaultRegistry.cs`, `Loading/FuseApplyTransaction.cs`, `Loading/FuseLoadReport.cs`.
...
Needs to be added:
- Log format policy.
- Console command docs for `/fuse.report`, `/fuse.loaded`, `/fuse.conflicts`, `/fuse.patches`.
...
Evidence:
- `FuseSettings.cs` currently exposes experimental early scene-path suppression and timeout.
...
Incomplete or risky:
- No general settings schema for FUSE or data packages.
- No user-facing config UI.

Needs to be added:
- Settings schema and policy.
- Clear separation between FUSE core settings and data package metadata.
...
Evidence:
- `FuseDataPackageDiscovery.cs` reads `FuseLoadPriority`, `FuseLoadAfter`, and `FuseLoadBefore`.
...
Already working:
- Data packages can express FUSE-native ordering separate from UMM `LoadAfter`.
- Missing dependencies and cycles are logged.
- Disabled packages are skipped and reported.

Incomplete or risky:
- Duplicate package IDs are warned during ordering; duplicate definition IDs within a package are hard validation errors in `FuseModLoader`.
...
- `schemas/FUSE_JSON_SCHEMA.md` is a useful schema reference.
- `schemas/fuse-mod.example.json` exercises many fields.
...
Evidence:
- `schemas/fuse-mod.example.json` provides a comprehensive document.
...
Needs to be added:
- Minimal "Hello FUSE" data package.
...
### Migration Path From Legacy Mods To FUSE

Current status: PARTIAL

Evidence:
- `tools/fuse_convert.py` handles route-style legacy JSON.
- `tools/convert_fuse_audio.py` handles horn/whistle/bell packs.
- Legacy component names are normalized in `FuseIndustryComponentTypes`.

Already working:
- Converted route packages can keep one output file per source file.
- Null legacy entries map to FUSE removals.
- SC/AMM handler families are mapped to FUSE types.
