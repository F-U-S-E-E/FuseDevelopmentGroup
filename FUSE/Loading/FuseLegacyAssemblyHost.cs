...
namespace FUSE.Loading
{
    internal static class FuseLegacyAssemblyHost
    {
            ...
            foreach (var manifest in DiscoverLegacyManifests(modsRoot))
            {
                ...
                foreach (var property in mixintos.Properties())
                {
                    ...
                    foreach (var entry in EnumerateMixintoEntries(property.Value))
                    {
                        ...
                        var mixintoPath = ResolvePackageFile(modsRoot, manifest.FolderPath, entry.Reference);
        ...
        internal static object ConvertModReferences(ModReference[] references, Type targetType)
        {
            var elementType = targetType?.IsArray == true
            ...
            if (targetType == null || targetType.IsInstanceOfType(result))
            {
                return result;
            }
            ...
            return targetType.IsInstanceOfType(list) ? list : result;
        }
        ...
        internal static bool IsLegacyReferencePresent(
            FuseLegacyAssemblyManifest manifest,
            ModReference reference,
            bool includeReplacementCapabilities = true)
        {
            ...
            if (includeReplacementCapabilities &&
                FuseReplacementCapabilityCatalog.IsProvided(reference.Id))
            {
                return true;
            }
            ...
            foreach (var packagePath in Directory.GetDirectories(modsRoot))
            {
                ...
                candidateVersions.Add(candidate.Version);
            }

            return AnyLegacyVersionMatchesReference(candidateVersions, reference);
        }

        internal static bool AnyLegacyVersionMatchesReference(
            IEnumerable<string> candidateVersions,
            ModReference reference)
        {
            foreach (var candidateVersion in candidateVersions ?? Enumerable.Empty<string>())
            {
                if (!FuseModRequirementResolver.TryParseVersion(candidateVersion, out var installedVersion))
                {
                    return true;
                }

                if ((reference.NotBefore == null || installedVersion.CompareTo(reference.NotBefore) >= 0) &&
        ...
        internal static string ResolvePackageFile(string modsRoot, string folderPath, string reference)
        {
            ...
            try
            {
                var relative = reference.Trim().Trim('"', '\'')
                ...
                var normalizedModsRoot = Path.GetFullPath(modsRoot)
                    .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
                var normalizedPackageRoot = Path.GetFullPath(folderPath)
                    .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
                if (!IsPathWithinRoot(normalizedModsRoot, normalizedPackageRoot))
                {
                    return string.Empty;
                }

                var resolved = Path.GetFullPath(Path.Combine(normalizedPackageRoot, relative));
                return IsPathWithinRoot(normalizedModsRoot, resolved) ? resolved : string.Empty;
            }
            catch (Exception)
            {
                return string.Empty;
        ...
        private sealed class HostedLegacyPlugin
        {
            ...
            public HostedLegacyPlugin(FuseLegacyAssemblyManifest manifest, Type type, object plugin)
            {
                ...
                {
                    var legacyInterface = type.GetInterfaces().FirstOrDefault(candidate =>
                        string.Equals(candidate.FullName, "Railloader.IUpdateHandler", StringComparison.Ordinal));
                    if (legacyInterface != null)
                    {
                        var map = type.GetInterfaceMap(legacyInterface);
                        var updateIndex = Array.FindIndex(map.InterfaceMethods, method =>
                            string.Equals(method.Name, nameof(IUpdateHandler.Update), StringComparison.Ordinal) &&
                            method.GetParameters().Length == 0);
                        if (updateIndex >= 0)
                        {
                            _reflectedUpdateMethod = map.TargetMethods[updateIndex];
                        }
                    }
                }
            }
