...
| `Assets/Tests/Rmroc451CompatibilityTests.cs` | Opt-in real-package harness for RMROC451's Tweaks and Things. With `FUSE_TEST_RMROC451_ZIP` and `FUSE_TEST_GAME_DIR` set, loads the third-party assembly inside Unity, constructs its RailLoader plugin through FUSE's compatibility context, and verifies that its Harmony targets resolve against the installed game API. |
