...
namespace FUSE.UnityTests
{
    ...
    /// assembly inside Unity, resolves its Railloader.Interchange reference
    /// through FUSE, constructs the plugin with FUSE's independent context,
    /// and resolves the plugin's real Harmony targets against the installed
    /// Railroader API.
    /// </summary>
    public sealed class Rmroc451CompatibilityTests
    {
        [Test]
        public void Real_plugin_resolves_hosts_and_patches_with_fuse_shim()
        {
            ...
            {
                Assert.Ignore(
                    "Opt-in real RMROC451 harness: set FUSE_TEST_RMROC451_ZIP to RMROC451.TweaksAndThings.zip.");
            }
            ...
            {
                Assert.Ignore(
                    "Opt-in real RMROC451 harness: set FUSE_TEST_GAME_DIR to a Railroader installation.");
            }
            ...
            try
            {
                ...
                Assert.IsTrue(typeof(PluginBase).IsAssignableFrom(pluginType));
                Assert.IsTrue(typeof(IUpdateHandler).IsAssignableFrom(pluginType));
                Assert.IsTrue(typeof(IModTabHandler).IsAssignableFrom(pluginType));

                var plugin = (PluginBase)Activator.CreateInstance(
                    pluginType,
                    context,
                    definition);
        ...
        private static void AssertHarmonyTargetsResolve(
            ...
            string category)
        {
            var harmony = new Harmony("FUSE.UnityTests.RMROC451.TargetAudit");
            ...
            Assert.IsTrue(patchClassCount > 0, "No RMROC451 Harmony patch classes were discovered.");
            Assert.IsTrue(resolvedTargetCount > 0, "No RMROC451 Harmony targets resolved against the installed game.");
        }
    }
}
